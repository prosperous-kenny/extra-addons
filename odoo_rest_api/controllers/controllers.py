...
class OdooAPI(http.Controller):
    ...
    def call_model_function(self, model, function, **post):
        ...
        if "values" in post:
            values = post["values"]
        if "order_id" in post:
            kwargs['sale_order_ids'] = [(6, 0, [post['order_id']])]
        if "session_id" in post:
            print("SessionId: "+str(post['session_id']))
    ...
    def call_obj_function(self, model, rec_id, function, **post):
        ...
        if "kwargs" in post:
            kwargs = post["kwargs"]
        obj = request.env[model].sudo().browse(rec_id).ensure_one()
    ...
    def get_model_rec(self, model, rec_id, **params):
        ...
        record = records.sudo().browse(rec_id).ensure_one()
    ...
    def post_model_data(self, model, **post):
        ...
        if "sale_order_ids" in post:
            data['sale_order_ids'] = [(6,0,[post['sale_order_ids']])]
        ...
        if "context" in post:
            ...
            if "sudo" in post:
               data['partner_id'] = request.env.user.partner_id
    ...
    def put_model_record(self, model, rec_id, **post):
        ...
        if "context" in post:
            ...
            rec = model_to_put.sudo().browse(rec_id).ensure_one
    ...
    def put_model_records(self, model, **post):
        ...
        if "context" in post:
            ...
            recs = model_to_put.sudo().search(filters)
        else:
